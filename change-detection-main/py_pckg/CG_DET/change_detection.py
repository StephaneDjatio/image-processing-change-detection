...
def process(S2_L2A_obj, L8_obj_list, target_shape, S2_shapes, tmp_path, tile, S2_th_bottom, S2_th_top, LS_th_bottom,
            LS_th_top, raster_targ, max_size):
    ...
    for idx, row in acq_datf_S2_L8.iterrows():
        ...
        nbr, mask = index_calc.get_nbr(acq_dict_S2_L8[row['acq_pathes']], tmp_path, target_crs, S2_shapes, tile,
                                       target_shape, rasterio.open(raster_targ).profile)
        ...
        if inc == 0:
            ...
            if inc == 1:
                ...
                change_add = np.where((change > 0) & (arr_to_analyse == 0) & (mask == False) & (changes_live != 1), 1,
                                      0)
                tmp = change
                change = np.where(mask == True, change,
                                  np.where(changes_live == 1, 1, np.where((tmp > 0) & (change_add == 1), tmp + 1, 0)))
