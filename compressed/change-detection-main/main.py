...
start_date, end_date = "20220601", "20220630"
...
for S2_tile in S2_tile_list : 
    ...
    if user_options["landsat_activation"].upper() == "TRUE" : 
        ...
        L8_tiles = L8.L8_corresponding_to_S2(S2_tile, current_main_dir + user_pathes_p["S2_shapes"], current_main_dir +user_pathes_p["L8_shapes"])
    ...
    dates, changes, acq_datf_S2_L8 = CG_DET.process(S2_L2A_S2C_obj, L8_acq_list, target_shape, current_main_dir + user_pathes_p["S2_shapes"], 
                                                    ...
                                                    S2_L2A_S2C_obj.arbo_dict[next(iter(S2_L2A_S2C_obj.arbo_dict))]["RED"], int(user_params["max_change_size_per_detect"]))
    

    csv_path = os.path.join(current_main_dir +user_pathes_p['chg_csv'], user_patter['chg_pat_csv'] +"_" + S2_tile + "_" + str(start_date) + "_" + str(end_date) + ".csv")

    chg_path_ras = os.path.join(current_main_dir + user_pathes_p['chg_rast'], user_patter['chg_pat_ras'] +"_" + S2_tile + "_" + str(start_date) + "_" + str(end_date) + ".tif")
    dat_path_ras = os.path.join(current_main_dir + user_pathes_p['dat_rast'], user_patter['dat_pat_ras'] +"_" + S2_tile + "_" + str(start_date) + "_" + str(end_date) + ".tif")

    chg_path_vec = os.path.join(current_main_dir + user_pathes_p['chg_vec'], user_patter['chg_pat_vec'] +"_" + S2_tile + "_" + str(start_date) + "_" + str(end_date) + ".shp")
    dat_path_vec = os.path.join(current_main_dir + user_pathes_p['chg_vec'], user_patter['dat_pat_vec'] +"_" + S2_tile + "_" + str(start_date) + "_" + str(end_date) + ".shp")

    merged_vec   = os.path.join(current_main_dir + user_pathes_p['chg_dat_vec'], user_patter['chg_pat_vec'] +"_" + S2_tile +"_" + str(start_date) + "_" + str(end_date) + ".shp")
    ...
    chg_path_ras, dat_path_ras = RASTER.clean(chg_path_ras, dat_path_ras, chg_path_ras, dat_path_ras, int(user_params["min_size_changes"]), int(user_params["min_occu_changes"]), current_main_dir + user_pathes_p['forest_msk'], current_main_dir + user_pathes_p['S2_shapes'], int(user_params["forest_mask_tampon"]), S2_tile, user_params["lc_code_forest_mask"]) 
    ...
    VECTOR.merge_vectors(chg_path_vec, user_patter["chg_attribute"], dat_path_vec, user_patter["dat_attribute"], merged_vec)
...
result_vec   = os.path.join(user_pathes_p['chg_dat_vec_f'], user_patter['chg_pat_vec'] +"_GABON_" + str(start_date) + "_" + str(end_date) + ".shp")
