    ...
    def __init__(self, l2a_dir, tile_name, start_date, end_date):
        ...
        for acquisitions in os.listdir(os.path.join(l2a_dir,self.tile)) : 
            ...
            if "SENTINEL2A" in acquisitions or "SENTINEL2B" in acquisitions :
                ...
                bands = [B2_band, B3_band, B4_band, B5_band, B6_band, B7_band, B8_band, B8A_band, B11_band, B12_band, MASK_10,   MASK_20,   EDG_R1,   EDG_R2,   QCK,   META]
                names = ["BLUE",  "GREEN", "RED", "veg_red_edg5", "veg_red_edg6", "veg_red_edg7", "NIR", "veg_red_edg8a", "SWIR1", "SWIR2", "MASK", "MASKS20", "NO_DATA", "NO_DATA20", "QCK", "META"]
        ...
        if len(self.unconsistant_data) != 0 :
            print("/ ! \ Warning :")
            print("Some of your datas seem to be unconsistant ! They will not be considered in the future processings. Here is the list of unconsidered acquisitions : ")
    ...
    def __init__(self, l2a_dir, tile_name, start_date, end_date):
        ...
        for acquisitions in os.listdir(os.path.join(l2a_dir,self.tile)) : 
            ...
            if "S2A_MSIL2A" in acquisitions or "S2B_MSIL2A" in acquisitions and not ".zip" in acquisitions:
                ...
                try : 
                    B2_band = glob(os.path.join(l2a_dir, self.tile, acquisitions, "GRANULE", "*", "IMG_DATA", "R10m", "*B02_10m.jp2"))[0]
                except : 
                    B2_band = "no_data"

                try :
                    B3_band = glob(os.path.join(l2a_dir, self.tile, acquisitions, "GRANULE", "*", "IMG_DATA", "R10m", "*B03_10m.jp2"))[0]
                except :
                    B3_band = "no_data"

                try :
                    B4_band = glob(os.path.join(l2a_dir, self.tile, acquisitions, "GRANULE", "*", "IMG_DATA", "R10m", "*B04_10m.jp2"))[0]
                except :
                    B4_band = "no_data"
                
                try :
                    B5_band = glob(os.path.join(l2a_dir, self.tile, acquisitions, "GRANULE", "*", "IMG_DATA", "R20m", "*B05_20m.jp2"))[0]
                except :
                    B5_band = "no_data"
                
                try :
                    B6_band = glob(os.path.join(l2a_dir, self.tile, acquisitions, "GRANULE", "*", "IMG_DATA", "R20m", "*B06_20m.jp2"))[0]
                except :
                    B6_band = "no_data"
                
                try :
                    B7_band = glob(os.path.join(l2a_dir, self.tile, acquisitions, "GRANULE", "*", "IMG_DATA", "R20m", "*B07_20m.jp2"))[0]
                except :
                    B7_band = "no_data"

                try :
                    B8_band = glob(os.path.join(l2a_dir, self.tile, acquisitions, "GRANULE", "*", "IMG_DATA", "R10m", "*B08_10m.jp2"))[0]
                except :
                    B8_band = "no_data"

                try :
                    B8A_band = glob(os.path.join(l2a_dir, self.tile, acquisitions, "GRANULE", "*", "IMG_DATA", "R20m", "*B8A_20m.jp2"))[0]
                except :
                    B8A_band = "no_data"

                try :
                    B11_band = glob(os.path.join(l2a_dir, self.tile, acquisitions, "GRANULE", "*", "IMG_DATA", "R20m", "*B11_20m.jp2"))[0]
                except :
                    B11_band = "no_data"

                try :
                    B12_band = glob(os.path.join(l2a_dir, self.tile, acquisitions, "GRANULE", "*", "IMG_DATA", "R20m", "*B12_20m.jp2"))[0]
                except :
                    B12_band = "no_data"

                try :
                    # A CHANGER 
                    if len(glob(os.path.join(l2a_dir, self.tile, acquisitions, "GRANULE", "*", "IMG_DATA", "R20m", "*SCL_20m_backup.jp2"))) != 0 : 
                        MASK_20 = glob(os.path.join(l2a_dir, self.tile, acquisitions, "GRANULE", "*", "IMG_DATA", "R20m", "*SCL_20m_backup.jp2"))[0]
                    else : 
                        MASK_20 = glob(os.path.join(l2a_dir, self.tile, acquisitions, "GRANULE", "*", "IMG_DATA", "R20m", "*SCL_20m.jp2"))[0]
                except :
                    MASK_20 = "no_data"

                try :
                    MASK_60 = glob(os.path.join(l2a_dir, self.tile, acquisitions, "GRANULE", "*", "IMG_DATA", "R60m", "*SCL_60m.jp2"))[0]
                except :
                    MASK_60 = "no_data"

                try :
                    EDG_R1 = glob(os.path.join(l2a_dir, self.tile, acquisitions, "GRANULE", "*", "IMG_DATA", "R20m", "*SCL_20m.jp2"))[0]
                except :
                    EDG_R1 = "no_data"

                try :
                    EDG_R2 = glob(os.path.join(l2a_dir, self.tile, acquisitions, "GRANULE", "*", "IMG_DATA", "R20m", "*SCL_20m.jp2"))[0]
                except :
                    EDG_R2 = "no_data"
                ...
                bands = [B2_band, B3_band, B4_band, B5_band, B6_band, B7_band, B8_band, B8A_band, B11_band, B12_band, MASK_20,   MASK_60,   EDG_R1,   EDG_R2,   QCK,   META]
                names = ["BLUE",  "GREEN", "RED", "veg_red_edg5", "veg_red_edg6", "veg_red_edg7", "NIR", "veg_red_edg8a", "SWIR1", "SWIR2", "MASK", "MASKS20", "NO_DATA", "NO_DATA20", "QCK", "META"]
                df = pd.DataFrame(columns=['names', 'pathes'])
...
class LANDSAT8(SENSOR) : 
    ...
    def __init__(self, l2a_dir, tile_name, start_date, end_date):
        ...
        for acquisitions in os.listdir(os.path.join(l2a_dir,self.tile)) : 
            ...
            if ("LC08_L2SP_" in acquisitions and not ".tar" in acquisitions) or ("LC09_L2SP_" in acquisitions and not ".tar" in acquisitions ):
...
def production(input_dir, tile_name, output_dir, L1C_files, unconsistant_L2A_list): 
    ...
    for files in L1C_files :
        ...
        to_process = glob(os.path.join(input, files, "*.SAFE"))[0]

        cmd = "docker run -v /mnt:/mnt atruffier/sen2cor --resolution 10 --GIP_L2A /mnt/raster/L2A_GIPP.xml " 
        cmd += to_process
