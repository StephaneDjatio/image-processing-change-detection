...
start_date, end_date = "20220301", "20220401"
...
for tiles in S2_tile_list:
    ...
    L1C_files_available = S2_L1C.search(tiles, start_date, end_date, f'{current_main_dir}' + user_pathes["L1C_files"],
                                        user_credentials['peps_id'], user_credentials['peps_pwd'])
    L1C_files_local = S2_L1C.get_local(tiles, start_date, end_date, user_pathes_s["L1C_pool"])
    L1C_to_download = S2_L1C.get_missing(L1C_files_local, L2A_corrupte_files, L2A_existing_files, L1C_files_available,
                                         user_pathes_s["L1C_pool"], tiles)

    S2_L1C.L1C_download(L1C_to_download, f'{current_main_dir}' + user_pathes["L1C_files"], user_pathes_s["L1C_pool"],
                        tiles, int(user_options["peps_attempts"]), user_options["peps_tempo"],
                        user_credentials['peps_id'], user_credentials['peps_pwd'])
